...
\noindent\textit{Lay-McDonald \S7.4; Justin 8.1.} Spectral Theorem (7.1) solutions are in Ch.~7.1. \textbf{SP4:} quiz \S7.4.5--12 and Justin 8.1; exam \S7.4.13--18.
...
\textbf{Step 2: Find the singular values.}
\[
\det(AA^T-\lambda I)
=
\begin{vmatrix}
...
\]
hence the eigenvalues of $AA^T$ are $25$ and $9$. Therefore the singular values are
\[
\sigma_1=5,\qquad \sigma_2=3.
\]

\textbf{Step 3: Find the left singular vectors.}
...
\textbf{Step 4: Find the right singular vectors.}

Use $v_i=\frac{1}{\sigma_i}A^Tu_i$.
...
To complete an orthonormal basis of $\mathbb{R}^3$, take a unit vector in $\operatorname{Nul}(A)$. Solving $Ax=0$ gives
\[
3x_1+2x_2+2x_3=0,\qquad
2x_1+3x_2-2x_3=0,
\]
which yields $x=(-2,2,1)$. Normalizing,
\[
v_3=\frac{1}{3}\begin{pmatrix}-2\\2\\1\end{pmatrix}.
\]
...
\textbf{Technology check:} \texttt{svd([3 2 2; 2 3 -2])} returns singular values $5$, $3$, and $0$, matching the hand computation.
\end{solution}
